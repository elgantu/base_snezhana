    ...
    return (
      <tbody>
       {fetchData.filter((post) => {  
          ...
          const idOrUid = fetchData[rowIndex].uid || fetchData[rowIndex].id
          return (
            <>
              <tr key={rowIndex} onClick={() => { navigate(`${pageUrl}/${idOrUid}/edit` || `${pageUrl}/${idOrUid}/edit`) }} className='border-1 border-top-0 border-start-0 border-end-0' style={{ backgroundColor: color }}>
                {
