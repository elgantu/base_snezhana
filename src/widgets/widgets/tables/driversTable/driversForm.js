...
const DriversForm = ({ apiUrl, method, SetAlert, scheme, editId, types }) => {  
    ...
    function updateValue(fieldName, fieldValue, depend, setValid) {  
     ...
     function SendData() {  
        const xxx = JSON.stringify(fetchDataById);
        const snddt = JSON.parse(xxx)
        console.log(snddt)
        let xdata = {}
        let keyCount = Object.keys(snddt).length
        for (let i = 0; i < keyCount; i++) {
            let keyName = Object.keys(snddt)[i]
            console.log(keyName)
            console.log(scheme.fields[keyName].type)
            //if (  (snddt[keyName] == '' && scheme.fields[keyName].type == 'reference')  ) {
                    xdata[keyName] = snddt[keyName]
                    console.log(snddt[keyName])
            //}

        }


        // for (let item in fetchDataById) { 
        //     if (!  (fetchDataById[item] == '' && scheme.fields[item].type == 'reference')  ) {
         
        //         console.log(123)
        //         // setSend(fetchDataById[item])
        //          xdata[item] = snddt[item]
        //     } 
        // }
        console.log(xdata)
        // let url;
